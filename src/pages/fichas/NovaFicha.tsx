...
const NovaFicha = () => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setImagemUrl(url);
      alert('Imagem do animal salva com sucesso!');
    } catch (error) {
      console.error('Erro no upload da imagem:', error);
      alert('Erro ao salvar a imagem do animal.');
    } finally {
  ...
  const adicionarVacina = () => {
    ...
    // Dose aplicada (com dataAplicacao preenchida)
    novaLista.push({
      nomeVacina: vacina.nome,
      dose: doseAtual,
      dataAplicacao: dataDose,
      proximaData: '',  // Dose aplicada não tem próxima
    });
    ...
    if (doseAtual < vacina.doses) {
      ...
      for (let i = doseAtual + 1; i <= vacina.doses; i++) {
        ...
        novaLista.push({
          ...
          dose: i,
          dataAplicacao: '',
          proximaData: dataFormatada,  // Chave correta para a agenda procurar
        });
      }
    }
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      <h1 className="text-4xl font-bold text-white text-center mb-10">Incluir Nova Ficha</h1>
